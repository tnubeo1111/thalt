...
func newGithubOAuth2Config(connector types.GithubConnector) oauth2.Config {
	return oauth2.Config{
		...
		Endpoint: oauth2.Endpoint{
			// ------VNPAYSSO------
			AuthURL:  fmt.Sprintf(GithubAuthPath),
			TokenURL: fmt.Sprintf(GithubTokenPath),
...
func (c *githubAPIClient) get(page string) ([]byte, string, error) {
	...
	request, err := http.NewRequest("GET", fmt.Sprintf("https://www.googleapis.com/oauth2/v2/userinfo%s", page), nil)
	if err != nil {
		return nil, "", trace.Wrap(err)
	}
	// ------VNPAYSSO------
...
const (
	...
	GithubAuthPath = "https://accounts.google.com/o/oauth2/auth"

	// GithubTokenPath is the GitHub token exchange endpoint
	// GithubTokenPath = "login/oauth/access_token"
	GithubTokenPath = "https://oauth2.googleapis.com/token"
